...
public class SisaankirjautumisServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(validiKayttaja) {
				session.setAttribute (VuokrausOhjelma.SESSION_ATTR_WEBUSER, kayttajaKannasta);
				request.getRequestDispatcher(VuokrausOhjelma.SECOND_PAGE).forward(request, response);
